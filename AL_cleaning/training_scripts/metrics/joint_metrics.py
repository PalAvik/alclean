...
from AL_cleaning.training_scripts.metrics.sample_metrics import SampleMetrics
from AL_cleaning.training_scripts.metrics.plots_tensorboard import (get_scatter_plot, plot_disagreement_per_sample,
                                                                 plot_excluded_cases_coteaching)
from AL_cleaning.training_scripts.transforms import ToNumpy
...
class JointMetrics():
    ...
    def __post_init__(self) -> None:
        self.reset()
        self.prediction_disagreement = np.zeros([self.num_samples, self.num_epochs], dtype=bool)
        self._initialise_dataset_properties()
        self.case_drop_histogram = np.zeros([self.num_samples, self.num_epochs + 1], dtype=bool)
